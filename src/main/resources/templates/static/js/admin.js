    ...
    */
    ...
    window.WeAdminEdit_notice = function(title, url, id, w, h) {
        ...
        $.ajax({
            ...
            success: function (result) {
                var jsonObj = JSON.parse(result);
                noticetitle = jsonObj.noticetitle;
                noticecontent = jsonObj.noticecontent;
                layer.open({
                    type: 2,
                    area: [w + 'px', h + 'px'],
                    fix: false, //不固定
                    ...
                    success: function(layero, index) {
                        ...
                        var body = layer.getChildFrame('body', index);//得到子页面层的BODY
                        body.contents().find("#L_ID").val(id);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_noticetitle").val(noticetitle);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_noticecontent").val(noticecontent);//将本层的窗口索引id传给子页面层的L_username中
                    },
                    error: function(layero, index) {
                        alert("aaa");
                    },
    ...
    /*弹出层+传递ID参数*/
    window.WeAdminEdit_security = function(title, url, id, w, h) {
        if(title == null || title == '') {
            title = false;
        };
        if(url == null || url == '') {
        ...
        var dorm = '11111';
        var electricity = '15140104';
        var dangerGood = '15140104';
        var lockDoor = '15140104';
        ...
        $.ajax({
            ...
            success: function (result) {
                var jsonObj = JSON.parse(result);
                dorm = jsonObj.dorm;
                electricity = jsonObj.electricity;
                dangerGood = jsonObj.dangerGood;
                lockDoor = jsonObj.lockDoor;
                layer.open({
                    type: 2,
                    area: [w + 'px', h + 'px'],
                    fix: false, //不固定
                    ...
                    success: function(layero, index) {
                        ...
                        body.contents().find("#L_electricity").val(electricity);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_dangerGood").val(dangerGood);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_lockDoor").val(lockDoor);//将本层的窗口索引id传给子页面层的L_username中
                    },
                    error: function(layero, index) {
                        alert("aaa");
                    },
    ...
    /*弹出层+传递ID参数*/
    window.WeAdminEdit_weisheng = function(title, url, id, w, h) {
        if(title == null || title == '') {
            title = false;
        };
        if(url == null || url == '') {
        ...
        $.ajax({
            ...
            success: function (result) {
                var jsonObj = JSON.parse(result);
                dorm = jsonObj.dorm;
                floor1 = jsonObj.floor1;
                balcony = jsonObj.balcony;
                bed = jsonObj.bed;
                layer.open({
                    type: 2,
                    area: [w + 'px', h + 'px'],
                    ...
                    success: function(layero, index) {
                        ...
                        body.contents().find("#L_ID").val(id);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_Dorm").val(dorm);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_floor").val(floor1);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_balcony").val(balcony);//将本层的窗口索引id传给子页面层的L_username中
                        body.contents().find("#L_bed").val(bed);//将本层的窗口索引id传给子页面层的L_username中
                    },
                    error: function(layero, index) {
                        alert("aaa");
                    },
